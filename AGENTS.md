...
- Sections size to their own content by default — no minimum viewport height is imposed. A short section (e.g. Contact) is exactly as tall as its content plus padding; a long one (e.g. Projects) simply grows. This was previously a `min-h-dvh` floor on every section; that floor was removed by explicit decision so section height reflects content instead of forcing every section to at least fill the screen. If a future section genuinely needs a viewport-height floor for a specific visual effect, add it via that section's own `className` override (not by reintroducing a global floor in `Section.tsx`), and use `dvh` rather than `vh`/`screen` if you do — `min-h-screen` is not used anywhere in this codebase, since `dvh` avoids the mobile-Safari layout jump caused by the address bar showing/hiding.
...
- Shared layout rules — responsive padding, the gap between a section's heading and its body content, and (if ever needed) a width constraint or height floor — belong in `Section.tsx`, not in individual `components/sections/*` files. A section component supplies only `id`, `heading`, and content; it must not hardcode its own margin/padding. No width constraint is applied by default today (content intentionally spans the full padded width per an earlier, explicit decision to drop `max-w-3xl`) — if one is ever needed, it's added to `Section.tsx` so every section picks it up uniformly, not per-file.
